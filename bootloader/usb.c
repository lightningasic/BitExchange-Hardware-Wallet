...
void language_state(const uint8_t *lang)
{
	
	if(!strncmp((const char *)lang, "zh", 2) || !strncmp((const char *)lang, "chinese", 7)) {
		language = CHINESE;
	}
	else
		language = ENGLISH;
...
static void hid_rx_callback(usbd_device *dev, uint8_t ep)
{
	...
	if (flash_state == STATE_OPEN) {
		if (msg_id == 0x0006) {		// FirmwareErase message (id 6)
			switch (language) {
				case CHINESE:
					layoutZhDialog(DIALOG_ICON_QUESTION, "放弃", "继续", NULL, "安装固件#?#", NULL, "请务必确认恢复卡", "还在#!#");
					break;
				default :
			...
			flash_state = STATE_END;
				switch (language) {
					case CHINESE:
						layoutZhDialog(DIALOG_ICON_WARNING, NULL, NULL, NULL, "固件安装中止#,#", NULL, "拔出你的钱包#,#", NULL);
						break;
					default :
						layoutDialog(DIALOG_ICON_WARNING, NULL, NULL, NULL, "Firmware installation", "aborted.", NULL, "You may now", "unplug your", "BitExchange.");
						break;
				}
			return;
		}
	...
	if (flash_state == STATE_FLASHSTART) {
		if (msg_id == 0x0007) {		// FirmwareUpload message (id 7)
			if (buf[9] != 0x0a) { // invalid contents
				...
				switch (language) {
					case CHINESE:
						layoutZhDialog(DIALOG_ICON_WARNING, NULL, NULL, NULL, "固件安装错误#,#", NULL, "拔出你的钱包#,#", "请重试#.#");
						break;
					default :
						layoutDialog(DIALOG_ICON_ERROR, NULL, NULL, NULL, "Error installing ", "firmware.", NULL, "Unplug your", "BitExchange", "and try again.");
						break;
				}
				return;
			}
			...
			if (flash_len > FLASH_TOTAL_SIZE + FLASH_META_DESC_LEN - (FLASH_APP_START - FLASH_ORIGIN)) { // firmware is too big
				...
				switch (language) {
					case CHINESE:
						layoutZhDialog(DIALOG_ICON_WARNING, NULL, NULL, NULL, "固件文件太大#,#请到", NULL, "bitexchange.com.hk", "获取安装文件#.#");
						break;
					default :
						layoutDialog(DIALOG_ICON_ERROR, NULL, NULL, NULL, "Firmware is too big.", NULL, "Get official firmware", "from", "bitexchange.com.hk", NULL);
						break;
				}
				return;
			}
	...
	if (flash_state == STATE_CHECK) {
		...
		switch (language) {
			case CHINESE:
				layoutZhDialog(DIALOG_ICON_QUESTION, "放弃", "继续", "校验固件", str[0], str[1], str[2], str[3]);
				break;
			default :
		...
		if (hash_check_ok) {
			switch (language) {
				case CHINESE :
					layoutZhDialog(DIALOG_ICON_OK, NULL, NULL, NULL, "固件安装成功#,#", NULL, "请拔出钱包#.#", NULL);
					break;
				default :
					layoutDialog(DIALOG_ICON_OK, NULL, NULL, NULL, "New firmware", "successfully installed.", NULL, "You may now", "unplug your", "BitExchange.");
					break;
			}
			send_msg_success(dev);
		} else {
			switch (language) {
				case CHINESE :
					layoutZhDialog(DIALOG_ICON_OK, NULL, NULL, NULL, "固件安装中止#,#", NULL, "请重新安装固件#.#", NULL);
					break;
				default :
