...
//	switch (language) {
//		case CHINESE:
//			layoutZhDialog(DIALOG_ICON_ERROR, NULL, NULL, NULL, "非官方固件#,#请拔", "出钱包#,#访问", "bitexchange.com.hk", "获得支持");
//			break;
//		default :
			layoutDialog(DIALOG_ICON_ERROR, NULL, NULL, NULL, "Unofficial firmware", "   aborted!", "Unplug your", "BitExchange and see", "our support page:", "bitexchange.com.hk");
//			break;
//	}
	system_halt();
}
...
void bootloader_loop(void)
{
	...
	oledDrawString(52, 0, "BitExchange");
...
{
	int broken = 0;
	if (memcmp((void *)FLASH_META_MAGIC, "BIEX", 4)) { // magic does not match
		broken++;
	}
	...
	//	switch (language) {
	//		case CHINESE:
	//			layoutZhDialog(DIALOG_ICON_ERROR, NULL, NULL, NULL, "固件已损坏#,#请拔", "出钱包#,#访问", "bitexchange.com.hk", "获得支持#.#");
	//			break;
	//		default :
				layoutDialog(DIALOG_ICON_ERROR, NULL, NULL, NULL, "Firmware appears", "to be broken.", "Unplug your", "BitExchange and see", "our support page:", "bitexchange.com.hk");
	//			break;
	//	}
		system_halt();
	}
...
int main(void)
{
	...
	uint16_t state_S = gpio_port_read(BTN_START);
	if (((state & BTN_PIN_YES) == BTN_PIN_YES || (state & BTN_PIN_NO) == BTN_PIN_NO) && ((state_S & BTN_PIN_START) == BTN_PIN_START)) {
