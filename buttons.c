...
{
	uint16_t state_S;
	static uint16_t last_state_S = BTN_PIN_START;

	state_S = gpio_port_read(BTN_START);

	if ((state_S & BTN_PIN_START) == 0) {	// START button is down
		if ((last_state_S & BTN_PIN_START) == 0) {		// last START was down
			if (button.YesDown < 2000000000) button.YesDown++;
			button.YesUp = false;
		} else {					// last START was up
			button.YesDown = 0;
			button.YesUp = false;
		}
	} else {				// START button is up
		if ((last_state_S & BTN_PIN_START) == 0) {		// last START was down
			button.YesDown = 0;
			button.YesUp = true;
		} else {					// last START was up
