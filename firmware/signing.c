...
static bool signing_check_prevtx_hash(void) {
	...
	if (memcmp(hash, input.prev_hash.bytes, 32) != 0) {
		fsm_sendFailure(FailureType_Failure_SyntaxError, "Encountered invalid prevhash");
		signing_abort();
		return false;
	}
...
static bool signing_check_output(TxOutputType *txoutput) {
	...
	if (txoutput->address_n_count > 0) {
		if (txoutput->has_address) {
			fsm_sendFailure(FailureType_Failure_SyntaxError, "Address in change output");
			signing_abort();
			return false;
		}
		/*
	...
	if (is_change) {
		if (change_spend == 0) { // not set
			...
			fsm_sendFailure(FailureType_Failure_SyntaxError, "Only one change output allowed");
			signing_abort();
			return false;
		}
	}
	...
	if (spending + txoutput->amount < spending) {
		fsm_sendFailure(FailureType_Failure_SyntaxError, "Value overflow");
		signing_abort();
		return false;
	}
...
static bool signing_sign_hash(TxInputType *txinput, const uint8_t* private_key, const uint8_t *public_key, const uint8_t *hash, uint8_t sighash) {
	...
	if (txinput->has_multisig) {
		...
		if (pubkey_idx < 0) {
			fsm_sendFailure(FailureType_Failure_SyntaxError, "Pubkey not found in multisig script");
			signing_abort();
			return false;
		}
...
static bool signing_sign_input(void) {
	...
	if (memcmp(hash, hash_outputs, 32) != 0) {
		fsm_sendFailure(FailureType_Failure_SyntaxError, "Transaction has changed during signing");
		signing_abort();
		return false;
	}
...
static bool signing_sign_segwit_input(TxInputType *txinput) {
	...
	if (txinput->script_type == InputScriptType_SPENDWITNESS
		...
		if (txinput->script_type == InputScriptType_SPENDWITNESS) {
			fsm_sendFailure(FailureType_Failure_SyntaxError, "Native segwit is disabled");
			signing_abort();
			return false;
		}
			...
			case STAGE_REQUEST_1_INPUT:
				...
				if (tx->inputs[0].script_type == InputScriptType_SPENDMULTISIG
					...
					if (idx1 > 0 && to.is_segwit == true) {
						fsm_sendFailure(FailureType_Failure_SyntaxError, "Mixing segwit and non-segwit inputs is not allowed");
						signing_abort();
						return;
					}
#endif
					...
					if (coin->has_forkid) {
						if (!tx->inputs[0].has_amount) {
							fsm_sendFailure(FailureType_Failure_SyntaxError, "SIGHASH_FORKID input without amount");
							signing_abort();
							return;
						}
					...
					if (!coin->has_segwit || !coin->segwit) {
						fsm_sendFailure(FailureType_Failure_SyntaxError, "Segwit not enabled on this coin");
						signing_abort();
						return;
					}
					// disable native segwit for now
					...
					if (!tx->inputs[0].has_amount) {
						fsm_sendFailure(FailureType_Failure_SyntaxError, "Segwit input without amount");
						signing_abort();
						return;
					}
					if (to_spend + tx->inputs[0].amount < to_spend) {
					...
					fsm_sendFailure(FailureType_Failure_SyntaxError, "Wrong input script type");
					signing_abort();
					return;
				}
				return;
